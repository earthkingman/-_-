- - ### 1. 과제 안내
    ...
    ![](https://images.velog.io/images/earthkingman/post/b49b3537-cd9c-444a-bf92-c6b8494198f2/image.png)
    ...
    - 해결
      ...
      여러 관점에서 저는 탈취가 됐을 때의 관점에서 바라보았습니다. 

      그리고 어떻게 탈취가 되는지 포스트맨과 스크립트를 작성해서 확인을 해보았습니다.

      포스트맨으로 먼저 로그인을 성공한 후에  발급받은 토큰과 세션아이디를 따로 빼서  curl을 날려보았습니다.

      서버는 두가지 방법 모두 정상적인 사용자로 인식하고 데이터를 제공했습니다. 

      근데 중요한점은 제가 알고있는 기술과 지식으로는 언제 탈취가됬고, 어떻게 탈취가 됬는지 알 수 있는 방법도, 털리는거도 막을 수가 없었습니다. 

      그러면 최대한 유효기간을 짧게두는게 나의 최고 방어방법이라고 생각했습니다. 

      근데 유효기간이 짧으면 사용자가 너무 불편합니다. 

      결론적으로 엑세스토큰은 만료시간을 최대한 짧게하고 아무런 정보가 없는 리프레시토큰을 비교적 유효기간을 길

      게해서  엑세스토큰을 재발급 받을수 있는 로직과  HTTPS를 통신을 사용해 통신 도중에 탈취를 막을 수 있게 설계
      ...
      - 세션 쿠키
        ...
        클라이언트에 저장되어 있는 Session ID를 가로챌 수 있습니다.

      - JWT (Access)

        클라이언트에 저장되어 있는 토큰을 가로챌 수 있습니다.
    ...
    ![image-20220108154008194](/Users/ji-park/Library/Application Support/typora-user-images/image-20220108154008194.png)
    ...
    - 첫번째 결과
      ...
      ![image-20220108155117594](/Users/ji-park/Library/Application Support/typora-user-images/image-20220108155117594.png)

    - 두번째 결과

      9500원

      ![image-20220108160032895](/Users/ji-park/Library/Application Support/typora-user-images/image-20220108160032895.png)

    #### select_for_update을 사용해 락을 거는 방법

    ![image-20220109215209520](/Users/ji-park/Library/Application Support/typora-user-images/image-20220109215209520.png)
